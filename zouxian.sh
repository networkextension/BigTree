...
while [ $SECONDS_PASSED -lt $MAX_WAIT_TIME ]; do
  ...
  if [ ! -z "$PID" ]; then
    ...
    -o "expression (void) [[[InputManager sharedInstance] objectForInputValue:1] setValue:@\"US\" forKey:@\"_deviceLocale\"]" \
    -o "expression (void) [[[InputManager sharedInstance] objectForInputValue:11] setValue:@NO forKey:@\"_hasExternalBootDrive\"]" \
